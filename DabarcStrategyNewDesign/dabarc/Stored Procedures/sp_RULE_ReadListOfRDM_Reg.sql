CREATE PROCEDURE  [dabarc].[sp_RULE_ReadListOfRDM_Reg] @IsRegister VARCHAR(5), @table_id INT AS
...
DECLARE @Sql_query			NVARCHAR(800),
 ...
 SELECT @db_id = database_id FROM t_TDM WHERE table_id =  @table_id
  
 SELECT @table_name = RTRIM(SUBSTRING(name,5,LEN(name))) FROM t_TDM WHERE table_id = @table_id
	...
	SET @Sql_query = '		
	  ...
	  UNION
			SELECT object_id, name, ''[Nuevo valor]'' ,''Vista'' FROM ' + @db_name + '.sys.views as cat 
				    	WHERE (name LIKE ''RDM[_]' + @table_name + '[_]%[_]ActVi'' OR 
					    name LIKE ''RDM[_]' + @table_name + '[_]%[_]InsVi'' OR
					    name LIKE ''RDM[_]' + @table_name + '[_]%[_]EliVi'') AND (SUBSTRING(name, 1, Len(name)-2) NOT IN (SELECT name
