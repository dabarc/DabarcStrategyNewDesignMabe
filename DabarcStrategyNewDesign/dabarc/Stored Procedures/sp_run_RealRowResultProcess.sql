...
CREATE PROCEDURE [dabarc].[sp_run_RealRowResultProcess] @ppath_unickey	NVARCHAR(80) AS 
		...
		-- Agregamos el Titulo
			...
			FROM (	 
				 SELECT			DISTINCT	
								...
								'<b>Executed By:</b> ' + RTRIM(Path_hUser)		AS Ejecuto
  ...
  IF (RTRIM(@IsType) <> 'INTE')
	...
	INSERT INTO #tempMessageH
		SELECT	'<b>Execution Time:</b> ' + 
				...
				RTRIM(CAST(SUM(CASE WHEN path_status = 4 THEN 1 ELSE 0 END) AS CHAR(4))) + ' of ' + 
   ...
   BEGIN
       INSERT INTO	 #tempMessageH
			SELECT	'<b>Execution Time:</b> ' + 
					...
					'<b>Correct Elements:</b> ' + 
					RTRIM(CAST(SUM(CASE WHEN d.path_status = 4 THEN 1 ELSE 0 END) AS CHAR(4))) + ' of ' + 
