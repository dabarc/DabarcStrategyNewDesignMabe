--EXECUTE [dabarc].[sp_run_Validate_Process_execute_RULE] 'RFF',2,'admin',null,0




CREATE PROCEDURE [dabarc].[sp_run_Validate_Process_execute_RULE] 
(
   ...
   @IsOnlyError		BIT
	...
	BEGIN
	  ...
	  FROM	#tmp_Sql_process_executeD d
			INNER JOIN t_TFF t ON d.path_table_padre_id = t.table_id 		    
	END
	...
	UPDATE d
	  SET	 d.path_table_padre_id = t.database_id
	  FROM	#tmp_Sql_process_executeD d
			INNER JOIN t_TDM t ON d.path_table_padre_id = t.table_id 	
	END
	...
	BEGIN
	  ...
	  SET 	 d.path_table_padre_id = t.database_id		 
	  FROM	 #tmp_Sql_process_executeD d
		INNER JOIN t_TFM m ON d.path_table_padre_id = m.table_id 	
