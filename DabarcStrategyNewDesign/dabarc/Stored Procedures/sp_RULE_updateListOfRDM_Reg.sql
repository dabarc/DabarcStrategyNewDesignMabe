CREATE PROCEDURE  [dabarc].[sp_RULE_updateListOfRDM_Reg] 	
	(	
	@table_id	int,
	@rule_id	int,
	@registered int,
...
WHERE table_id = @table_id
 ...
 ----------------------------------------------------------------------------------
  ...
  IF (LEN(RTRIM(@strError)) > 0)
   ...
   END
    ...
    INSERT INTO t_RDM(name, create_date, table_id, status, report_type, registered) 
 ...
 BEGIN
    ...
    EXECUTE [dabarc].[sp_RULE_updateListOfNumber_Reg] 'TDM', @table_id, @register_user
END

   --IF (@registered = 1)
   --BEGIN
   --   DECLARE @name VARCHAR(50)
   --   SELECT @name = name FROM dabarc.t_RDM WHERE table_id = @table_id AND rule_id = @rule_id
